...
public:
    ...
    //40 30 35 80 100
    //30 35 40 80 100 -> 40 -> 30
    unordered_map<int,int> idx;
    Node* build(int &i,int pre[],int n,int l,int r){
        if(i>=n) return NULL;
        if(l>r) return NULL;
        ...
        int m = idx[root->data];
        root->left = build(i,pre,n,l,m-1);
        root->right = build(i,pre,n,m+1,r);
    ...
    Node* post_order(int pre[], int size)
    {
        ...
        int i = 0;
        return build(i,pre,size,0,size-1);
    }
};
